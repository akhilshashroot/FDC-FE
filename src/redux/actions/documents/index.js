...
// Saving image in server to get URL for implementing image upload feature in text editor

export const AddNewDocumentImage = (data) => {
  ...
  return (dispatch) => {
    dispatch({
      type: "ADD_DOCUMENT_IMAGE_REQUEST",
    });

    return performRequest("post", "/api/documentation/imageStore", headers, formData)
      .then((response) => {
        if (response.data.response_code === 200) {
          dispatch({
            type: "ADD_DOCUMENT_IMAGE_SUCCESS",
            payload: response,
          });
          return response;
        }
...
export const UpdateDocument = (docid, data) => {
  ...
  return (dispatch) => {
    ...
    return performRequest(
      "put",
      `/api/documentation/${docid}/update`,
      headers,
      data
    )
      .then((response) => {
...
export const deleteDocument = (docid) => {
  ...
  return async (dispatch) => {
    ...
    return await performRequest(
      "delete",
      `/api/documentation/${docid}/delete`,
      headers
    )
      .then((response) => {
      ...
      .catch((error) => {
        dispatch({ type: "DELETE_DOCUMENT_FAILURE", payload: error.response });
      });
  };
};