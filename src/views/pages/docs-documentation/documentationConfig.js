...
import { Card, CardBody, Row, Col, Button, Badge, Label, FormGroup } from "reactstrap";
...
import { AddNewDocument, GetDocumentList, GetADocument, UpdateDocument, deleteDocument } from "../../../redux/actions/documents";
...
const Added = () => toast.success("Document Added Successfully", { transition: Zoom })
...
const Duplicate = () => toast.warning("The Document Name has already been taken..", { transition: Zoom })
const Updated = () => toast.info("Document Updated Successfully", { transition: Zoom })
const DeletedSuccess = () => toast.success("Document Deleted Successfully.", { transition: Zoom })
...
class DocumentationConfig extends React.Component {
    ...
    handleUserChange = user => {
        ...
        if (user.value !== 'all') {
            let templist = []
            templist = this.props.DocumentationList && this.props.DocumentationList.allDocumentData.filter((value) => value.user_role_id == user.value)
    ...
    componentDidMount() {
        ...
        if (this.props.DocumentationList && !this.props.DocumentationList.allDocumentData) {
    ...
    toggleDocDeleteModal = (id = false) => {
        if (id) {
            this.setState({ toggleDeleteModal: !this.state.toggleDeleteModal, docid: id })
        } else {
            this.setState({ toggleDeleteModal: !this.state.toggleDeleteModal, docid: "" })
        }
    }

    render() {
        const { docsView, docsAdd, docid, docsUpdate, toggleDeleteModal, user_role, loading, user, remote, empty } = this.state
        const norecord = {
            background: "#dcdbdb",
            minHeight: "50px",
        }
                ...
                {(loading === true) ? (
                    <Spinner color="primary" className="reload-spinner" />
                        ...
                        <>
                                    ...
                                    <>
                                        {(user_role === "Admin" || "Senior Sales") &&
                                            <>
                                                <Col sm="4">
                                                    <FormGroup>
                                                        ...
                                                        <Select
                                                            ...
                                                            options={this.convertUser(this.props.userList && this.props.userList.prevData)}
                                                            isSearchable={true}
                                                        />
                                                    </FormGroup>
                                                </Col>
                                                ...
                                                <Col sm={`${remote ? "8" : "8"}`}>
                                                    <div className="data-list-header d-flex justify-content-between flex-wrap mb-1 mt-2" style={{ float: "right" }}>
                            ...
                            {docsView === false && docsAdd === false && docsUpdate === false &&
                                <Row>
                                    {(this.props.DocumentationList && this.props.DocumentationList.allDocumentData && this.props.DocumentationList.allDocumentData.length > 0) &&
                                        this.props.DocumentationList.allDocumentData.map((value, index) =>
                                            <>
                                                {user.value === value.user_role_id &&
                                                    <Col md="4" sm="6" className="search-content" key={index}>
                                                        <Card>
                                                            <CardBody className="text-center">
                                                                <span className="link" onClick={() => this.docsClick(value.id)}>
                                                                    <img
                                                                        src={docs}
                                                                        alt="documentation"
                            ...
                            {this.props.DocumentationList && this.props.DocumentationList.allDocumentData && this.props.DocumentationList.allDocumentData.length === 0 &&
                                <Row>
                                    <Col sm="12">
                                        <Card style={norecord}>
                                            <CardBody className="text-center">
                                                <h4 style={{ color: "red" }}>Sorry there are no documents to display!</h4>
                                                <ToastContainer />
                                            </CardBody>
                                        </Card>
                                    </Col>
...
export default connect(mapStateToProps, {
    AddNewDocument, GetDocumentList, GetADocument, UpdateDocument, deleteDocument, getInitialData, GetUserPrivilege
})(DocumentationConfig)